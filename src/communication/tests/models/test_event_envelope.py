...
class TestEventEnvelope:
    ...
    def test_created_at_delegates_to_event(self) -> None:
        e = make_event()
        env = EventEnvelope(event=e, metadata=make_metadata(), priority=EventPriority.NORMAL)
        assert env.created_at == e.occurred_at

    def test_event_type_property(self) -> None:
        env = EventEnvelope(event=make_event(), metadata=make_metadata(), priority=EventPriority.NORMAL)
    ...
    def test_is_retry_true(self) -> None:
        ...
        env = EventEnvelope(event=make_event(), metadata=m, priority=EventPriority.NORMAL)
    ...
    def test_to_dict(self) -> None:
        env = EventEnvelope(event=make_event(), metadata=make_metadata(), priority=EventPriority.CRITICAL)
        d = env.to_dict()
        assert "event" in d
