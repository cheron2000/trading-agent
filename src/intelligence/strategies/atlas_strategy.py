...
STEP 1 — Confirm Regime Gate:
  - trending  -> Trend-following only (ride direction VWAP/MACD confirm)
  - ranging   -> Mean-reversion only (fade toward VWAP from band extremes)
  - volatile  -> Breakout continuation only (cap max confidence at 70)
  - crisis    -> Capital preservation only (HOLD / flat)
  If the setup belongs to a different family than the regime permits -> action MUST be HOLD.

STEP 2 — Score Confluence (3 layers):
  a) Trend layer: MACD line/signal/histogram agree with direction?
  b) Momentum layer: RSI in correct zone (35-65 pullback for trend; <30 or >70 for mean-reversion)?
  c) Volatility layer: Price position in Bollinger Bands supports direction?
  Count agreement: 3/3 = Strong. 2/3 = Marginal. 0-1/3 = HOLD.

STEP 3 — Risk & Reward Gate:
  - Stop-loss: 1.5-2x ATR from price.
  - Take-profit: minimum 2:1 reward:risk ratio required. If < 2:1 -> HOLD.

STEP 4 — Assign Calibrated Confidence (0-100):
  - 0-59  : 0-2/3 layers agree or regime mismatch -> HOLD
  - 60-79 : 3/3 layers agree in trending/ranging regime -> BUY/SELL
  - 80-100: 3/3 layers agree + strong regime confidence -> STRONG BUY/SELL
